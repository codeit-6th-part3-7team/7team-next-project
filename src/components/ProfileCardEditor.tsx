...
export default function ProfileCardEditor({ profileData, profileImage, handleChangeProfile }: ProfileCardProps) {
  return (
    <section className="flex h-[511px] flex-col items-center gap-5 rounded-xl border-none px-[34px] py-4 shadow-lg shadow-gray-200 md:h-[388px]">
      <Image className="size-[62px] rounded-full object-cover md:size-[72px]" src={profileImage || ic_profile_skeleton} alt="프로필이미지" />
      <div className="flex flex-col gap-2 md:mt-3">
        <div className="flex flex-col gap-4 md:grid md:grid-cols-2 md:gap-x-10">
          ...
          {Object.entries(profileData).map(([key, value]) => (
            <div key={key} className="flex h-[45px] items-center gap-5">
